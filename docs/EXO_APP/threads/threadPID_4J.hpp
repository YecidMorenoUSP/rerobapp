...
class ThreadPID_4J: public ThreadType{
    private:
        fromPID_4J data;
        fromESP32 dataESP32;
        fromSetParameters dataSetParameters;
        fromJonathanESP32FSR dataJonathanESP32FSR;
    ...
    public:       
        ...
        void _setup(){
            ...
            hip_r->local.encoder_in_Q = 12800;
            hip_r->local.Kp = 150;
            hip_r->local.Ki = 1.2;
            hip_r->local.Kd = 0;
            hip_r->local.Kv = 0;
            hip_r->local.Bv = 0;
            hip_r->local.Ts = (1.0 / 200.0);
            hip_r->local.VEL_MAX = 5000;
            hip_r->local.ks = 0;
            hip_r->local.fnc_Torque = &EXO_joint::T_quadraticSpringBooker;
            hip_r->local.N = 101;
            hip_r->local.TORQUE_MAX = 10;

            hip_l->local.encoder_in_Q = 12800;
            hip_l->local.Kp = 120;
            hip_l->local.Ki = 1.2;
            hip_l->local.Kd = 0;
            hip_l->local.Kv = 0;
            hip_l->local.Bv = 0;
            hip_l->local.Ts = (1.0 / 200.0);
            hip_l->local.VEL_MAX = 5000;
            hip_l->local.ks = 266;
            hip_l->local.fnc_Torque = &EXO_joint::T_linearSpring;
            hip_l->local.N = 101;
            hip_l->local.TORQUE_MAX = 5;
        ...
        void _firstLoop() {
            ...
            SetParameters->getData(&dataSetParameters);
            knee_r->local.Kv = dataSetParameters.Kv;
            knee_r->local.Bv = dataSetParameters.Bv;
            knee_r->vars[0].theta_ld = dataSetParameters.line1;

            ESP32_1->getData(&dataESP32);
        ...
        void _loop(){
            ...
            //ESP32_1->getData(&dataESP32);
            SetParameters->getData(&dataSetParameters);
            //ESP32_Jonathan->getData(&dataJonathanESP32FSR); //Jonathan
            knee_r->vars[0].theta_ld = dataSetParameters.line1;

            //dataJonathanESP32FSR.FSR[5] // jonathan

            /*if (time_index == 0) {
                hip_r->local.ZERO_L = dataESP32.hip_r;
                hip_l->local.ZERO_L = dataESP32.hip_l;
            }   */        

            knee_r->updatePosEncoder();
            knee_r->calc_torque();
            static float t_d_k_r = 0;
            //sineShape_test(0, .3, &t_d_k_r);
            ...
            hip_r->setPosEncoder_l(dataESP32.hip_r);
            hip_r->calc_torque();
            static float t_d_h_r = 0;
            //squareShape_test(&state,&t_d,10);
            //sineShape_test(0, .3,&t_d_h_r);
            //hip_r->vars[0].torque_d = t_d_h_r;
            hip_r->calc_vel();
            hip_r->setVelocity(0);

            hip_l->updatePosEncoder();
            hip_l->setPosEncoder_l(dataESP32.hip_l);
            hip_l->calc_torque();
            static float t_d_h_l = 0;
            //squareShape_test(&state,&t_d,10);
            //sineShape_test(0, .3, &t_d_h_l);
            //hip_l->vars[0].torque_d = t_d_h_l;
            hip_l->calc_vel();
            hip_l->setVelocity(0);


            _mtx.lock();   
            data.angle_knee_r = knee_r->vars[0].theta_l;
                // setdata 
            _datalog[time_index][0] = timer->get_current_time_f();
            _datalog[time_index][0 * 10 + 1] = knee_r->vars[0].theta_m;
            ...
            _datalog[time_index][0 * 10 + 7] = 0.0f;
            ...
            _datalog[time_index][2 * 10 + 1] = hip_r->vars[0].theta_m;
            _datalog[time_index][2 * 10 + 2] = hip_r->vars[0].theta_l;
            _datalog[time_index][2 * 10 + 3] = hip_r->vars[0].theta_c;
            _datalog[time_index][2 * 10 + 4] = hip_r->vars[0].torque_d;
            _datalog[time_index][2 * 10 + 5] = hip_r->vars[0].torque_l;
            _datalog[time_index][2 * 10 + 6] = hip_r->vars[0].omega_m;
            _datalog[time_index][2 * 10 + 7] = 0.0f;
            _datalog[time_index][2 * 10 + 8] = 0.0f;
            _datalog[time_index][2 * 10 + 9] = 0.0f;
            _datalog[time_index][2 * 10 + 10] = 0.0f;

            _datalog[time_index][3 * 10 + 1] = hip_l->vars[0].theta_m;
            _datalog[time_index][3 * 10 + 2] = hip_l->vars[0].theta_l;
            _datalog[time_index][3 * 10 + 3] = hip_l->vars[0].theta_c;
            _datalog[time_index][3 * 10 + 4] = hip_l->vars[0].torque_d;
            _datalog[time_index][3 * 10 + 5] = hip_l->vars[0].torque_l;
            _datalog[time_index][3 * 10 + 6] = hip_l->vars[0].omega_m;
            _datalog[time_index][3 * 10 + 7] = 0.0f;
            _datalog[time_index][3 * 10 + 8] = 0.0f;
            _datalog[time_index][3 * 10 + 9] = 0.0f;
            _datalog[time_index][3 * 10 + 10] = 0.0f;
